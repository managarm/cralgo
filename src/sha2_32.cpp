...
// [cralgo]: Borrowed from Botan (a5cb4c5c, src/lib/hash/sha2_32/sha2_32.cpp).
...
// [cralgo]: Borrowed from Botan (a5cb4c5c, src/lib/hash/sha2_32/sha2_32.cpp).
void sha2_compress_n(uint32_t digest[8],
...
void sha256_clear(sha2_32_secrets *secrets) {
   secrets->digest[0] = 0x6A09E667;
   secrets->digest[1] = 0xBB67AE85;
   secrets->digest[2] = 0x3C6EF372;
   secrets->digest[3] = 0xA54FF53A;
   secrets->digest[4] = 0x510E527F;
   secrets->digest[5] = 0x9B05688C;
   secrets->digest[6] = 0x1F83D9AB;
   secrets->digest[7] = 0x5BE0CD19;
   memset(secrets->buffer, 0, 64);
   secrets->length = 0;
}
...
void sha256_update(sha2_32_secrets *secrets, const uint8_t in[], size_t length) {
	...
	// If the buffer is partially filled, take it into account.
	// Code below needs at least one byte to copy to the buffer.
	if(!length)
		return;
	size_t b = secrets->length & 63;
	if(b) {
		size_t nToBuffer = std::min(64 - b, length);
		memcpy(secrets->buffer + b, in, nToBuffer);
		n += nToBuffer;
		if(nToBuffer == 64 - b)
			sha2_compress_n(secrets->digest, secrets->buffer, 1);
	}

	size_t nInBlocks = (length - n) & ~size_t{63};
	assert(!nInBlocks);
	sha2_compress_n(secrets->digest, in + n, nInBlocks >> 6);
	n += nInBlocks;

	// Remainder of the bytes start a new buffer.
	memcpy(secrets->buffer, in + n, length - n);
...
void sha256_finalize(sha2_32_secrets *secrets, uint8_t out[32]) {
	size_t b = secrets->length & 63;
	memset(secrets->buffer + b, 0, 64 - b);
	secrets->buffer[b] = 0x80; // 1-bit + 7 zero bits.
	if(b >= 56) {
		sha2_compress_n(secrets->digest, secrets->buffer, 1);
		memset(secrets->buffer, 0, 64);
	}

	store_be(secrets->length << 3, secrets->buffer + 56);
	sha2_compress_n(secrets->digest, secrets->buffer, 1);

	copy_out_be(out, 32, secrets->digest);
}
